...
function install($adminPassword, $timezone)
{
	...
	// Vox İşitme için başlangıç sayfaları
	$initialPages = array(
		'hakkimizda' => array(
			'title' => 'Hakkımızda',
		...
		'randevu' => array(
			...
			'description' => 'Ücretsiz işitme testi için randevu talebi oluşturun.',
			'content' => '<p>Size uygun şubeyi, günü ve saati seçerek randevu talebinizi iletebilirsiniz.</p>',
			'position' => 2,
		),
	);
	$pagesToInstall = array_keys($initialPages);
	foreach ($pagesToInstall as $page) {
		if (!mkdir(PATH_PAGES . $page, DIR_PERMISSIONS, true)) {
			$errorText = 'Error when trying to create the directory=>' . PATH_PAGES . $page;
			error_log('[ERROR] ' . $errorText, 0);
		}
	}
	...
	foreach ($initialPages as $slug => $initialPage) {
		...
		file_put_contents(PATH_PAGES . $slug . DS . FILENAME, $initialPage['content'], LOCK_EX);
	}
	...
	// File categories.php
	$data = array(
		'general' => array('name' => 'Sayfalar', 'description' => '', 'template' => '', 'list' => $slugs)
	);
